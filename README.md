...
- 四个房间、八波敌人，最终挑战宿敌 Boss；清关后自动弹出强化选择，再通过传送门进入下一关。敌人血量随房间 ×2 递增（×1/×2/×4/×8），与玩家火力增长镜像。
- 开局二选一武器：焚化者喷火枪（锥形灼烧场，每秒对锥内所有敌人结算一次伤害，火焰天然烧穿一切）或游骑兵步枪（精准弹道 · 中远点射，可堆穿透），选择定义整局构筑。
- 每清一间自动暂停**三选一**强化（无需拾取）：所选属性 **×2**（弹丸/伤害/射速/穿透/弹匣）——极端堆叠或均衡发展都行，一局 3 次。
- 掩体、闪避、爆炸桶、近战追击、远程弹幕、重甲冲锋。
- 单人行动或 AI 战友协作。**尚未实现真人多人联机。**
- 中文 / English 即时切换，支持保存语言偏好，切换时保留当前行动状态。
- 不做金币和商店。每次行动开局二选一武器，每关补满生命、当前弹匣与闪避，保留本次行动已获得的强化。

这是可玩原型，后续内容由三名组员逐步扩展。谁在游戏基础上交付了哪些功能，见[功能清单与负责人](docs/FEATURES.md)。“90 分”是品质目标，需通过真实试玩验证，不是本项目已经获得的评分。
...
| 武器与强化构筑                    | 开局在近程喷火枪与远程步枪中二选一，战斗中通过道具箱二选一获得加法型强化（弹丸、伤害、射速、穿透、弹匣）形成构筑；这仍是核心成长方式。 |
...
必须保留 2D 俯视角、射击闯关 PvE、单人与协作方向、开局选武器与道具箱强化构筑等核心玩法。
